...
class InputHandler {
	...
	get left() {
		...
		if (this.gamepad && this.gamepadStd) {
			l = l || this.gamepad.buttons[StdButton.DP_LEFT].pressed;
			l = l || this.gamepad.axes[0] < -STICK_THRESHOLD;
		}
		return l;
	}
	...
	get right() {
		...
		if (this.gamepad) {
			...
			r = r || this.gamepad.axes[0] > STICK_THRESHOLD;
		}
		return r;
	}
	...
	get up() {
		...
		if (this.gamepad) {
			...
			u = u || this.gamepad.axes[1] < -STICK_THRESHOLD;
		}
		return u;
	}
	...
	get down() {
		...
		if (this.gamepad) {
			...
			d = d || this.gamepad.axes[1] > STICK_THRESHOLD;
		}
		return d;
	}

	get use() {
		let u = this.keyboard.pressed(Key.SPACE);
		if (this.gamepad) {
			u = u || this.gamepad.buttons[StdButton.FACE_DOWN].pressed;
		}
		return u;
	}
